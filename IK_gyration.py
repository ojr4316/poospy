...
a_irl = np.array([a_i[0][0] + a_i_mod[0][0],
                  a_i[1][0] + a_i_mod[1][0],
                  a_i[2][0] + a_i_mod[2][0],
                  a_i[3][0] + a_i_mod[3][0],
                  a_i[4][0] + a_i_mod[4][0],
...
#def gen_beta(i):
#    return np.arccos((seg2**2 + l_i2abs[i]**2 - seg3**2)/(2 * seg2 * l_i2abs[i])) - (rho[i] + phi[i])

#beta = np.array([gen_beta(0), gen_beta(1), gen_beta(2), gen_beta(3), gen_beta(4), gen_beta(5)])

#print("Beta " + str(beta))

#def gen_gamma(i):
#    return np.pi - np.arccos(seg2**2 + seg3**2 - l_i2abs[i]**2)/(2*seg2*seg3)

#gamma = np.array([gen_gamma(0), gen_gamma(1), gen_gamma(2), gen_gamma(3), gen_gamma(4), gen_gamma(5)])
